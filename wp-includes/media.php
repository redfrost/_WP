...
function img_caption_shortcode($attr, $content = null) {
	...
	return '<div ' . $id . 'class="caption ' . esc_attr($align) . '" style="width: ' . (10 + (int) $width) . 'px">'
...
function gallery_shortcode($attr) {
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( $captiontag && trim($attachment->post_excerpt) ) {
			$output .= "
				<{$captiontag} class='caption-text gallery-caption'>
...
function wp_audio_shortcode( $attr ) {
	...
	if ( ! empty( $src ) ) {
		...
		if ( ! in_array( $type['ext'], $default_types ) )
			return sprintf( '<a class="embedded-audio" href="%s">%s</a>', esc_url( $src ), esc_html( $src ) );
		$primary = true;
...
function wp_video_shortcode( $attr ) {
	...
	if ( ! empty( $src ) ) {
		$type = wp_check_filetype( $src, wp_get_mime_types() );
		if ( ! in_array( $type['ext'], $default_types ) )
			return sprintf( '<a class="embedded-video" href="%s">%s</a>', esc_url( $src ), esc_html( $src ) );
		$primary = true;
		array_unshift( $default_types, 'src' );
	} else {
		foreach ( $default_types as $ext ) {
